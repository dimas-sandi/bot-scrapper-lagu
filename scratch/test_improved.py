...
def classify_artist(artist):
    ...
    # 1. Check Malaysian known list first
    ...
    if wiki_id:
        # Check foreign country match first to avoid false positive "Indonesia" on foreign pages
        ...
        if any(x in wiki_id for x in ['jepang', 'tokyo', 'osaka', 'j-pop', 'jpop', 'anime']):
            return "Jepang"
        if any(x in wiki_id for x in ['tiongkok', 'cina', 'taiwan', 'hong kong', 'hongkong', 'mandarin', 'c-pop']):
            return "Mandarin"
        if any(x in wiki_id for x in ['amerika', 'inggris', 'serikat', 'london', 'new york', 'barat', 'kanada', 'australia', 'jerman', 'prancis']):
            return "Barat"
            
        # Check Indonesian music/band indicators
...
for art in test_artists:
    # Use encode/decode with ignore to print safely in console
    gen = classify_artist(art)
    print(f"Artist: {art} -> Genre: {gen}")
